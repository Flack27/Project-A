...
buffs: [endure, DEF+, shield, immunity, guard, invincibility]
debuffs:
  - taunt
  - ATK-
recovery:
...
**Intercept** — Single target active. Apply a [Guard] to target ally. While active, Guardian takes X% of all damage directed at that target. Undispellable — cannot be stripped or cleansed from either character.
...
**Shield Wall** — AoE active. Apply [Taunt] and [ATK-] to all enemies. Apply a DEF-scaling [Shield] to self.
...
**Unyielding** — When a hit would reduce the guardian's hp below 10% gain [Invincibility].
